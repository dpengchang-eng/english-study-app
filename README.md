...
The Expo app in `mobile/` is a **separate product**. v1 is convert-only: Home → Result. Convert runs on the phone (Firebase AI Logic / Gemini API). The project is on Firebase Spark, so Cloud Functions are not used. The web flashcard app is unchanged.
